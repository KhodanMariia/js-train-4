...
function padString(original, length, padWith, side) {
  ...
  if (side == "start") {
    ...
    return original.padStart(length, padWith); //до певної довжини length, вставляючи символи padWith на початку

    // Якщо "end", використовуємо padEnd
  } else if (side == "end") {
    // Використовуємо метод padEnd(), щоб доповнити рядок string
    return original.padEnd(length, padWith); //до певної довжини length, вставляючи символи padWith на в кінці

    // Якщо бік не вказано або вказано неправильно,
  } else {
    return "Error: side should be either 'start' or 'end'"; // повертаємо повідомлення "Error: side should be either 'start' or 'end'"
  }
}
...
function trimString(original, side) {
  ...
  if (side == "start") {
    ...
    return "Error: side should be either 'start', 'end' or 'both'"; //повертаємо повідомлення "Error: side should be either 'start', 'end' or 'both'"
  }
}
...
function convertCase(original, caseType) {
  ...
  if (caseType == "upper") {
    ...
    return "Error: caseType should be either 'upper' or 'lower'"; // повертаємо повідомлення "Error: caseType should be either 'upper' or 'lower'"
  }
}
